...
function spawnBalls(material) {
    ...
    gameItems.forEach((item, i) => {
        ...
        const body = new CANNON.Body({
            mass: 10,
            material: material,
            position: new CANNON.Vec3((Math.random() - 0.5) * 10, 5 + i * 2, (Math.random() - 0.5) * 10),
            angularDamping: 0.1,
            linearDamping: 0.1
        });
        body.addShape(shape);
        body.velocity.set((Math.random() - 0.5) * 15, (Math.random() - 0.5) * 5, (Math.random() - 0.5) * 15);
...
function spawnSingleBall(item, material) {
    const radius = 2.2;
    const geometry = new THREE.SphereGeometry(radius, 32, 32);
    const outlineGeo = new THREE.SphereGeometry(radius * 1.05, 32, 32);
    const outlineMat = new THREE.MeshBasicMaterial({
        color: styles.outlineColor,
        side: THREE.BackSide
    });
...
function updateStatusList(material) {
    ...
    gameItems.forEach((item) => {
        ...
        const randomStatusColor = palette[Math.floor(Math.random() * palette.length)];
...
function setupUIEvents(material) {
    ...
    btnModalClose.addEventListener("click", () => uiEditModal.classList.remove("show"));
    ...
    btnResetCancel.addEventListener("click", () => uiResetModal.classList.remove("show"));
    ...
    btnShuffle.addEventListener("click", () => {
        balls.forEach((obj) => {
            if (obj.isShrinking)
                return;
            obj.body.wakeUp();
            obj.body.velocity.set((Math.random() - 0.5) * 60, 40 + Math.random() * 20, (Math.random() - 0.5) * 60);
            obj.body.angularVelocity.set((Math.random() - 0.5) * 20, (Math.random() - 0.5) * 20, (Math.random() - 0.5) * 20);
        });
    });
        ...
        setTimeout(() => {
            if (currentDrawnBall) {
                if (checkRemove.checked) {
                    ...
                    currentDrawnBall.body.velocity.set((Math.random() - 0.5) * 20, 10, (Math.random() - 0.5) * 20);
                }
...
function onMouseDown(e) {
    if (e.target.closest(".bottom-left-controls") ||
...
function animate() {
    ...
    if (!uiResult.classList.contains("show") &&
    ...
    for (let i = balls.length - 1; i >= 0; i--) {
        ...
        const SAFE_LIMIT = 15;
        if (Math.abs(obj.body.position.x) > SAFE_LIMIT)
            obj.body.velocity.x *= -1;
        if (Math.abs(obj.body.position.z) > SAFE_LIMIT)
            obj.body.velocity.z *= -1;
    }
    world.step(1 / 60);
    balls.forEach((obj) => {
        if (obj.isShrinking) {
