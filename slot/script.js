...
function createReels() {
  ...
  const innerCylinderGeo = new THREE.CylinderGeometry(
    REEL_RADIUS - 0.1,
    REEL_RADIUS - 0.1,
    REEL_WIDTH + 1,
    32
  );
  const innerCylinderMat = new THREE.MeshBasicMaterial({
...
function prepareSpinResult() {
  ...
  if (bonusModeActive) {
    ...
    console.log(
      `Spins since last check: ${spinsSinceLastBonusCheck}/${nextBonusCheck}`
    );
...
function stopReel(index) {
  ...
  const centerTileIdx =
    ((centerIndexRaw % REEL_SEGMENTS) + REEL_SEGMENTS) % REEL_SEGMENTS;
  ...
  if (winLines.length > 0) {
    ...
    if (firstWin.symbolId === 0) {
      ...
      statusText.innerText =
        noWinMessages[Math.floor(Math.random() * noWinMessages.length)];
    } else {
